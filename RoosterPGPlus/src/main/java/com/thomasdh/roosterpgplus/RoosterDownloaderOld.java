    ...
    protected void onProgressUpdate(Exception... e) {
        if (context.get() != null) {
            ...
            if (type == RoosterViewFragment.Type.DOCENTENROOSTER)
                new RoosterBuilderOld(context.get(), (ViewPager) rootView.get().findViewById(R.id.rooster_viewPager), week, type).buildLayout(new RoosterWeek(null, context.get()));
            if (type == RoosterViewFragment.Type.KLASROOSTER)
                new RoosterBuilderOld(context.get(), (ViewPager) rootView.get().findViewById(R.id.rooster_viewPager), week, type, klas).buildLayout(new RoosterWeek(null, context.get()));
        }
    }
...
//                RoosterWeek roosterWeek = new RoosterWeek(null, context.get());
                ...
                if (context != null && rootView.get() != null) {
                    if (type == RoosterViewFragment.Type.PERSOONLIJK_ROOSTER)
                        new RoosterBuilderOld(context.get(), (ViewPager) rootView.get().findViewById(R.id.rooster_viewPager), week, type).buildLayout(new RoosterWeek(string, context.get()));
                    if (type == RoosterViewFragment.Type.DOCENTENROOSTER)
                        new RoosterBuilderOld(context.get(), (ViewPager) rootView.get().findViewById(R.id.rooster_viewPager), week, type).buildLayout(new RoosterWeek(string, context.get()));
                    if (type == RoosterViewFragment.Type.KLASROOSTER)
                        new RoosterBuilderOld(context.get(), (ViewPager) rootView.get().findViewById(R.id.rooster_viewPager), week, type, klas).buildLayout(new RoosterWeek(string, context.get()));
                }
            }
        }
    }
